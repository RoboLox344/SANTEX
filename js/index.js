...
const ROOT_PRODUCTSSALE = document.getElementById('CartSale');
//cсоздание карточек хиты продаж
class ProductsHits{

    render(){
      let HtmlCatalogHits = '';
      CATALOG__HITS__Base.forEach(({id, name, price, img, art, sale}) => {
      ...
      ROOT_PRODUCTSHIT.innerHTML = HtmlCatalogHits;
    }
}

const ProductsHitsPage = new ProductsHits();
ProductsHitsPage.render();

//cсоздание карточек товар по скидке
class ProductsSale{

  render(){
    let HtmlCatalogSale = '';
    CATALOG__SALE__Base.forEach(({id, name, price, img, art, sale}) => {
    ...
    ROOT_PRODUCTSSALE.innerHTML = HtmlCatalogSale;
  }
}

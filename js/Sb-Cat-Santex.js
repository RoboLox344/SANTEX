const ROOT_SUBCT_SANTEX = document.getElementById('SubСtSantex');

const SUBCATALOG_OTOPLENIE = [
    {
        id:'001',
...
//подкаталог
class SubCatalogSantex {

    render(){
        let HtmlSubCatalogOtoplenie = '';
        SUBCATALOG_OTOPLENIE.forEach(({id, name}) => {

            HtmlSubCatalogOtoplenie +=`

                    <div id='${id}' class="sub-catalog--item">
                        <a href="#">${name}</a> 
                    </div>
        ...
        ROOT_SUBCT_SANTEX.innerHTML = HtmlSubCatalogOtoplenie;
    };
};
const SubCSantexPage = new SubCatalogSantex();
SubCSantexPage.render();
...
//roots
const ROOT_PRODUCTS = document.getElementById('catalog-wp');
...
const SANTEX__Base = [
  ...
  const arrCards = SANTEX__Base.slice(0, COUNT_SHOW_CART);
  console.log(arrCards)
  CreadeCart(arrCards);
  var newCartCount = arrCards.length;

  const buttonMore = document.querySelector('.more'); 
  buttonMore.addEventListener('click', function(){

    newCartCount +=4;
    var dopCart = SANTEX__Base.slice(newCartCount-4, newCartCount);
    CreadeCart(dopCart)
    if(SANTEX__Base.length == newCartCount){
  ...
  function OpacityButtun(){
    if (SANTEX__Base.length < 12){
        buttonMore.style.display = 'none';
    }
    
  };
  OpacityButtun(SANTEX__Base);
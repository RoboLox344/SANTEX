const ROOT_SUBCT_DACHA = document.getElementById('SubСtDacha');
...
//подкаталог
class SubCatalogDacha {

    render(){
        let HtmlSubCatalogDacha = '';
        ...
        ROOT_SUBCT_DACHA.innerHTML = HtmlSubCatalogDacha;
    };
};
const SubCDachaPage = new SubCatalogDacha();
SubCDachaPage.render();
...
const DACHA__Base = [
  ...
  const arrCards = DACHA__Base.slice(0, COUNT_SHOW_CART);
  ...
  buttonMore.addEventListener('click', function(){
    ...
    newCartCount +=4;
    var dopCart = DACHA__Base.slice(newCartCount-4, newCartCount);
    CreadeCart(dopCart)
    if(DACHA__Base.length == newCartCount){
  ...
  function OpacityButtun(){
    if (DACHA__Base.length < 12){
        buttonMore.style.display = 'none';
    }
    
  };
  OpacityButtun(DACHA__Base);